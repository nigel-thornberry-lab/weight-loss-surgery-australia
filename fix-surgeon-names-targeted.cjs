...
/**
 * Targeted Surgeon Name Fix Script
 * 
...
function cleanSurgeonName(originalName) {
  ...
  if (originalName.length < 50 && !originalName.includes(' - ') && !originalName.includes(' | ') && !originalName.includes('Centre') && !originalName.includes('Surgery')) {
    return originalName;
  }
  
  // Extract the actual surgeon name from common patterns
...
function processCSVFile(inputFile, outputFile) {
  ...
  fs.writeFileSync('surgeon-name-mapping-targeted.json', JSON.stringify(mappingData, null, 2));
  
  console.log(`✅ CSV processing complete!`);
  console.log(`📁 Output file: ${outputFile}`);
  console.log(`📁 Name mapping: surgeon-name-mapping-targeted.json`);
...
// Main execution
function main() {
  console.log('🎯 Starting Targeted Surgeon Name Fix...\n');
  
  const inputFile = 'surgeons-complete-data.csv';
  const outputFile = 'surgeons-complete-data-fixed.csv';
  ...
  try {
    ...
    console.log('\n✅ Targeted surgeon name fix complete!');
    console.log('\n📋 Summary:');
    ...
    const examples = Array.from(nameMapping.entries()).slice(0, 5);
