...
/**
 * Clean Surgeon Names Script
 * 
 * This script cleans up surgeon names to only include the title (Dr, Mr, Prof, etc.) 
 * and the actual name, removing business names, locations, and other unnecessary text.
 */
...
function processCSVFile(inputFile, outputFile) {
  ...
  fs.writeFileSync('surgeon-name-mapping.json', JSON.stringify(mappingData, null, 2));
  
  console.log(`✅ CSV processing complete!`);
  console.log(`📁 Output file: ${outputFile}`);
  console.log(`📁 Name mapping: surgeon-name-mapping.json`);
  console.log(`📊 Processed ${dataLines.length} records`);
...
// Main execution
function main() {
  console.log('🧹 Starting Surgeon Name Cleanup Process...\n');
  
  const inputFile = 'surgeons-complete-data.csv';
  const outputFile = 'surgeons-complete-data-cleaned.csv';
  ...
  try {
    ...
    console.log('\n🔄 Next steps:');
    console.log('   1. Review the cleaned names in surgeon-name-mapping.json');
