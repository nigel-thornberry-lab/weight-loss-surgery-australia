...
const files = walk(root).filter((f) => {
  ...
  // Exclude top-level listing pages like "melbourne.astro", "sydney.astro"
...
for (const file of files) {
  ...
  const photo = /surgeon_photo\s*:\s*["'][^"']+["']/.test(text) || /"surgeon_photo"\s*:\s*["'][^"']+["']/.test(text);
  const place = /place_id\s*:\s*["'][A-Za-z0-9_-]+["']/.test(text) || /"place_id"\s*:\s*["'][A-Za-z0-9_-]+["']/.test(text);
