...
const enhancedData = JSON.parse(fs.readFileSync(ENHANCED_DATA, 'utf-8'));
const seoData = JSON.parse(fs.readFileSync(SEO_DATA, 'utf-8'));
...
function generateProfile(surgeon, enhanced, seo) {
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div class="md:col-span-1">
            {surgeon.surgeon_photo ? (
              <img src={surgeon.surgeon_photo} alt={surgeon.name} class="w-full rounded-2xl shadow-lg" loading="eager" />
            ) : (
...
function main() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Only generate if we have enhanced data
    if (!enhanced || !seo) continue;

    const content = generateProfile(surgeon, enhanced, seo);
  ...
  console.log(`\n🎉 Generated ${generated} enhanced profiles!`);
}
